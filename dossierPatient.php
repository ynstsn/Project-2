<?php
    ...
    if(isset($_POST['id_patient'])){ ## On vérifie  si on a associé une prescripteur a une ordonnace 
        $_SESSION['Temp_Asso']['id_patient'] = $_POST['id_patient']; ## Si oui on l'enregistre et on l'envoie
    ...
    if(isset($_POST['add'])){ ## Si on doit ajouter
        if(isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['sexe']) && isset($_POST['dob']) && isset($_POST['adresse']) && isset($_POST['cp']) && isset($_POST['ville'])){ ## On verifie si tout les champs sont entrées
            $nouveau = new Patient( DAO::UNKNOWN_ID, $_POST['nom'], $_POST['prenom'], $_POST['adresse'], $_POST['cp'], $_POST['ville'], $_POST['dob'], $_POST['sexe']);
            $PatientDAO->save($nouveau); ## On l'enregistre dans la BDD
        ...
        $unPatient = $PatientDAO->getOne($_POST['id']); ## On récupère les information de l'opération dans la BDD
    } elseif(isset($_POST['id_del'])){ ## Si on veut supprimer 
        $unPatient = $PatientDAO->getOne($_POST['id_del']);
        $PatientDAO->delete($unPatient); ## On supprime le type dans la BDD
        $_SESSION['log_err'] = array(
            'type' => "success",
        ...
        if(isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['sexe']) && isset($_POST['dob']) && isset($_POST['adresse']) && isset($_POST['cp']) && isset($_POST['ville'])){ ## On verifie si tout les champs sont entrées
            $nouveau = new Patient( $_POST['id_update'], $_POST['nom'], $_POST['prenom'], $_POST['adresse'], $_POST['cp'], $_POST['ville'], $_POST['dob'], $_POST['sexe']);
            $PatientDAO->save($nouveau); ## On mets a jour les informations dans la BDD
            $_SESSION['log_err'] = array(
                'type' => "success",
...
<head>
    <title>Gestion Patient · Médicalife</title>
    <meta charset="UTF-8">
...
<body>
    ...
    <main>
        ...
        <form action="" method="POST" class="action" id="action">
            ...
            <?php else: ?>
            <button type="submit" id="button_add" name="add_btn" class="sign2">Ajouter un patient</button>
            <?php endif ?>
        </form>
        <div class="all_info_tab">
            <form method="POST" action="" id="table" class="table">
                <table id="example" class="dataTable">
                    ...
                    <tbody>
                        <!-- On affiche toutes les patients -->
                        <?php foreach($PatientDAO->getAll() as $patient): ?>
                        <tr id="ligne_<?= $patient->NoClient ?>" onclick="ouvrirProfile(<?= $patient->NoClient ?>)">
                            <td data-label=ID><?= $patient->NoClient ?></td>
                            <td data-label=Nom><?= $patient->NomClient ?></td>
                            <td data-label=Prénom><?= $patient->PrenomClient ?></td>
                            <td data-label=Sexe><?= $patient->checkSexe() ?></td>
                            <td data-label="Né(e) le"><?= formatDate($patient->DateNaissance) ?></td>
                            <td class="adress" data-label=Adresse><?= $patient->Adresse ?>, <?= $patient->CodePostal ?>
                                <?= $patient->Ville ?></td>
                        </tr>
                        <?php endforeach ?>
                    </tbody>
                ...
                <form method="POST" action="" enctype="multipart/form-data" class="slider" id="slider">
                        <div class="div_pp">
                            <?php if(isset($openView)): ?>
                                <img id="image_pp" src="<?= check_file_existe($_POST['id'], "client"); ?>" alt="pp"> <!-- On verifie queelle image afficher -->
                            <?php else: ?>
                                <div id="uploadForm" class="uploadForm">
                                    ...
                                    <label class="labal_pp" for="imageInput">
                                        <img id="previewImage" class="previewImage" src="<?php if(isset($openUpdate)){echo check_file_existe($_POST['id'], "client");} else { echo "Media/img/inco.png"; } ?>" alt="pp"> <!-- On affiche l'image qui viens d'etre envoyé -->
                        ...
                        <div class="info" id="info_client">
                            ...
                            <?php else: ?>
                                ...
                                <input type="text" name="nom" id="nom" maxlength="50" <?php if(isset($openUpdate)){echo "value='".$unPatient->NomClient."'";} ?> required> <!-- Si on veut modifier on mets la valeurs deja connu -->
                            <?php endif ?>
                            </h2>
                            <h2>Prénom :
                            <?php if(isset($openView)): ?>
                                <?= $unPatient->PrenomClient ?>
                            <?php else: ?>
                                <input type="text" name="prenom" id="prenom" maxlength="50" <?php if(isset($openUpdate)){echo "value='".$unPatient->PrenomClient."'";} ?> required>
                            <?php endif ?>
                            </h2>
                            <h2>Sexe :
                            <?php if(isset($openView)): ?>
                            ...
                            <h2>Né(e) le :
                            <?php if(isset($openView)): ?>
                                <?= formatDate($unPatient->DateNaissance) ?>
                            <?php else: ?>
                                <input type="date" name="dob" id="dob" maxlength="50" <?php if(isset($openUpdate)){echo "value='".$unPatient->DateNaissance."'";} ?> required>
                            <?php endif ?>
                            </h2>
                            <h2>Adresse : 
                                <?php if(isset($openView)): ?>
                                    <?= $unPatient->Adresse ?>
                                <?php else: ?>
                                    <input type="text" name="adresse" id="adresse" maxlength="150" <?php if(isset($openUpdate)){echo "value='".$unPatient->Adresse."'";} ?> required>
                                <?php endif ?>
                           </h2>
                           <h2>Code postal :
                                <?php if(isset($openView)): ?>
                                    <?= $unPatient->CodePostal ?>
                                <?php else: ?>
                                    <input type="text" name="cp" id="cp" pattern="[0-9]+" maxlength="6" <?php if(isset($openUpdate)){echo "value='".$unPatient->CodePostal."'";} ?> required>
                                <?php endif ?>
                           </h2>
                           <h2>Ville :
                                <?php if(isset($openView)): ?>
                                    <?= $unPatient->Ville ?>
                                <?php else: ?>
                                    <input type="text" name="ville" id="ville" maxlength="20" <?php if(isset($openUpdate)){echo "value='".$unPatient->Ville."'";} ?> required>
                                <?php endif ?>
                           </h2>
                        </div>
                        <div class="div_btn">
                            ...
                            <form action="" method="POST" class="form_delet" id="delet_civil">
                                <input type="hidden" id="id_patient" name="id_patient" value="<?= $unPatient->NoClient ?>/<?= $unPatient->NomClient ?> <?= $unPatient->PrenomClient ?>">
                                <button class="sign2" type="submit">Associer</button>
                            </form>
                            <?php else: ?>
                            <form action="" method="POST" class="form_delet" id="update_personnel">
                                <input type="hidden" name="id" id="id" value="<?= $unPatient->NoClient ?>">
                            ...
                            <form action="" method="POST" class="form_delet" id="delet_personnel">
                                <input type="hidden" name="id_del" id="id_del" value="<?= $unPatient->NoClient ?>">
                            ...
                            <input type="hidden" name="id_update" id="id_update" value="<?= $unPatient->NoClient ?>" required>
                            <button class="sign2" type="button"
